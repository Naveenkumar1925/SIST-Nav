...
// This is your REAL graph: only these nodes count as path nodes
...
// 👉 Build a set of REAL graph node ids (only nodes used in edges)
const graphNodeIds = new Set();
campusConnections.forEach(([from, to]) => {
    graphNodeIds.add(from);
    graphNodeIds.add(to);
});
...
// --------- DIJKSTRA (only uses graphNodeIds) ---------

function dijkstra(startNodeId, endNodeId) {
    // Make sure both are in the actual graph
    if (!graphNodeIds.has(startNodeId) || !graphNodeIds.has(endNodeId)) {
        console.error("Start or end node is not part of the graph:", startNodeId, endNodeId);
    ...
    while (current !== null) {
        ...
        if (path.length > graphNodeIds.size + 1) {
...
// 👉 findClosestNode NOW uses ONLY graphNodeIds
function findClosestNode(position) {
    ...
    console.log("Closest graph node:", closestNodeId, "Distance:", minDistance, "meters");

    if (minDistance > 1000) {
        console.warn("User is too far from any known graph node");
        return null;
    }
...
function handleNewPosition(position, isSingleUpdate = false) {
    ...
    if (destSelect && destSelect.value !== "Select Destination") {
        calculateRoute();
...
function calculateRoute() {
    ...
    generateDirections(result, pathCoords, destination);
    updateRoadNameVisibility();
...
function generateDirections(result, pathCoords, destination) {
    ...
    // Start step: current position → first node
    ...
    // Final step: last node → destination
